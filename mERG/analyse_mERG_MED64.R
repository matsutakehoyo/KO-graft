...
for (i in seq_along(files)){
	...
	for (ch in seq(1:64)){
		...
		if(nrow(erg_ch)){
			...
			erg_ch_peak <- analyse_peaks(x=erg_ch$time, y=erg_ch$filter, flash_time=flash_time,
			                             plot=FALSE, w=peak_w * 2, span=peak_span * 2)	
			p <- plot_peaks(x=erg_ch$time, peak=erg_ch_peak, event_pos=flash_time) +
				scale_x_continuous(limits = c(flash_time - 500, flash_time + 1500), expand = c(0, 0))
			ggsave(paste0(filename, "/channels_fit/", sprintf("CH%02d", ch), "_cd.png"), p, width=3, height=2)


			#test if detected peaks are in reasonable location
			peak_times_neg <- erg_ch$time[erg_ch_peak$i_min[erg_ch_peak$signal_neg]]-flash_time
			peak_times_pos <- erg_ch$time[erg_ch_peak$i_max[erg_ch_peak$signal_pos]]-flash_time

			condition_c <- which(a_delay < peak_times_neg & peak_times_neg < c_delay)
			condition_d <- which(b_delay + 100 < peak_times_pos & peak_times_pos < d_delay)

			if (length(condition_c)){
				cabs <- min(erg_ch_peak$y_hat[which(erg_ch$time %in% (peak_times_neg[condition_c] + flash_time))])
				camplitude <- cabs
				ctime <- peak_times_neg[peak_times_neg %in% (erg_ch$time[which(erg_ch_peak$y_hat==cabs)] - flash_time)]
			} else {
				cabs <- NA
				camplitude <- NA
				ctime <- NA
			...
			if (length(condition_d)){
				dabs <- max(erg_ch_peak$y_hat[which(erg_ch$time %in% (peak_times_pos[condition_d] + flash_time))])
				damplitude <- dabs
				dtime <- peak_times_pos[peak_times_pos %in% (erg_ch$time[which(erg_ch_peak$y_hat==dabs)] - flash_time)]
			} else {
				dabs <- NA
				damplitude <- NA
				dtime <- NA
			...
			erg_peaks <- bind_rows(erg_peaks,
							tibble( 
								   ...
								   c_amplitude = camplitude, 
								   d_amplitude = damplitude,
								   a_time = atime,
								   b_time = btime,
								   c_time = ctime, 
								   d_time = dtime,
	...
	erg_peaks <- erg_peaks %>% 
		gather(key=key, value=value, a_abs:d_time) %>%
	...
	for (ch in seq(1:64)){	
		...
		p_ch <-  ggplot(mea_data %>% dplyr::filter(channel==ch), aes(x = time - flash_time, y = filter)) +
			...
			scale_x_continuous(limits = c(-50, d_delay), expand = c(0, 0)) +
			xlab("time after flash(ms)") +
			ylab("response (uV)") +
	...
	p_summary_amplitude <- ggplot(erg_peaks, aes(x=peak, y=amplitude)) +
		...
		facet_wrap(~peak, scales="free", ncol=4) +
		xlab("peaks") +
		ylab("Amplitude(mV)") +
		ggtitle("Micro ERG peak amplitudes") +
		basic_style
	ggsave(paste0(filename, "/mERG amplitude summary.pdf"), width = 7, height = 6)
	...
	p_summary_timetopeak <- ggplot(erg_peaks, aes(x=peak, y=time)) +
		...
		basic_style
	ggsave(paste0(filename, "/mERG time to peak sumamry.pdf"), width = 7, height = 6)
